<rn:meta controller_path="standard/search/SearchButton2" 
         js_path="custom/search/SearchButton2OnChange" 
...
<div id="rn_<?=$this->instanceID;?>" class="rn_SearchButton2">
    <? if ($this->data['attrs']['icon_path']):?>
        <input type="image" class="rn_SubmitImage" <?=tabIndex($this->data['attrs']['tabindex'], 1);?> src="<?=$this->data['attrs']['icon_path'];?>" alt="<?=$this->data['attrs']['icon_alt_text'];?>" title="<?=$this->data['attrs']['label_button'];?>"/>
    <? else:?>
        <input type="submit" class="rn_SubmitButton" <?=tabIndex($this->data['attrs']['tabindex'], 1);?> value="<?=$this->data['attrs']['label_button'];?>" />
    <? endif;?>
    <? if($this->data['isIE']): ?>

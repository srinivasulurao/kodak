 /* Originating Release: February 2014 */
RightNow.Widget.LoginForm2 = function(data, instanceID){
    this.data = data;
    this.instanceID = instanceID;
    this._widgetContent = document.getElementById("rn_" + this.instanceID);
    if(this._widgetContent)
    {
...
RightNow.Widget.LoginForm2.prototype = {
    ...
    _onLoginResponse : function(type, result)
    {
        ...
        if(this.instanceID == result.w_id)
        {
            if(result.success == 1)
            {
                ...
                var redirectUrl = this._getRedirectUrl(result);
                if(YAHOO.env.ua.ie && YAHOO.env.ua.ie < 9 && RightNow.Text.beginsWith(redirectUrl, '/ci/fattach/get/'))
                    this._widgetContent.innerHTML = RightNow.Text.sprintf(RightNow.Interface.getMessage("PLS_CLICK_HREF_EQS_PCT_S_THAN_S_MSG"), redirectUrl);
                else
    ...
    _onSubmit : function()
    {
        ...
        else if(eo.data.username.indexOf('"') > -1)

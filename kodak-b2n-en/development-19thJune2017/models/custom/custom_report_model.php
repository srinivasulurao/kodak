...
class Custom_Report_model extends \RightNow\Models\Base {
 ...
 *************************************************************************************************/
    ...
    protected function getOtherKnowledgeBaseData()
    {
        ...
        {
            $this->returnData['topics'] = $this->CI->model('Topicbrowse')->searchBrowseTree($this->returnData['search_term']);
        }
    ...
    */
    protected function keywordSearchUpdate()
    {
        ...
        if($term && $term != '' && !$alreadyUpdated && !isSpider())
        {
...
//            if ($isComplex)
//            {
//                $queryResults = Api::rnkl_query_parse(0x0040, $searchTerm, $incident);
//            }
//            else
//            {
//                $queryResults = Api::rnkl_query_parse(0x0004, $searchTerm, $incident);
//            }
    ...
    protected function formatExternalSearchData()
    {
        foreach ($this->returnData['data'] as &$row)
        {
            if($this->appliedFormats['truncate_size'] > 0)
                $row[2] = Text::truncateText($row[2], $this->appliedFormats['truncate_size']);
            echo "Hell1222o";
            die;
...
//        {
//            $text = Api::print_text2str($answerRow[ans_soln], $options);
//            $text = Api::utf8_trunc_nchars($text, $maximumTextLength);
//        }
        sql_free($si);
 ...
 *****************************************************************************************/
    ...
    {
        $dataArray = array();
        while ((count($dataArray) < $maxResults))
        {
    ...
    * order is
    * 1 - spider
    * 2 - per_page attribute in widget
    * 3 - user profile set from the setFilters function
    * 4 - report
    * 5 - default of 15
    ...
    */
    ...
    {
        if (isSpider() && !$this->appliedFilters['sitemap'])
        {
            // Sitemap enabled spiders will hit the sitemap page, but there is a limit
            // on page size for the sitemap spec; therefore, don't show everything if
            // it's a sitemap, even if it is a spider.
            return 0x7fffffff; // Max Signed Int -- show everything on one page
        }
                ...
                if ( (isset($this->viewDataColumnDefinition['col_item'.$j]['hidden']) &&  ($this->viewDataColumnDefinition['col_item'.$j]['hidden'] === 0)) || !$formatAsHtml)
                {
                    ...
                    if ($bindType == BIND_MEMO)
                    {
                        ...
                        if ($columnDefinition === "{$this->answerTableAlias}.solution" || $columnDefinition === "{$this->answerTableAlias}.description" || $this->reportIsTypeExternalDocument)
                        {
                            ...
                            if($this->appliedFormats['truncate_size'] > 0)
                                $temp[$count] = truncateText($temp[$count], $this->appliedFormats['truncate_size']);
                        }
                        else
                        {
                            $temp[$count] = $row[$j];
...
//                        }
    ...
    protected function filtersToSearchArgs()
    {
        ...
        //TODO: removed this to prevent report from adding contact id filter
    ...
    */
    protected function preProcessClusterTreeFilter()
    {
        if(view_tbl2alias($this->reportID, TBL_CLUSTER2ANSWERS))
        {
            if($this->appliedFilters['keyword']->filters->data && !$this->appliedFilters['parent']->filters->data->val)
            {
                ...
                $bestClusterID = $this->CI->model('Topicbrowse')->getBestMatchClusterID($this->appliedFilters['keyword']->filters->data);
                if($bestClusterID)
                {
	...
	public function getReportDataByID($report_id)
	{
		$menu_array = array();
		...
		if($nrows){
			...
			{
				$menu_array[] = array('ID'=>$row['Countryid'],'LookupName'=>$row['Country']);
			}

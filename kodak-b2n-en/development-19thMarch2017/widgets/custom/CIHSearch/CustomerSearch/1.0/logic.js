  ...
  _resetDisplay: function () {
	...
	_selectIRepair:function(evt,eo){
		...
		var eo = new RightNow.Event.EventObject();
                ...
                eor.data.sds = cell.sds.value;

                eor.data.knum = cell.ID.value;

                eor.data.sn = cell.SN.value;

                eor.data.sp = cell.sp.value;

                eor.data.rp = cell.rp.value;

                eor.data.equip_id = cell.compID.value;

                eor.data.sap_prod_id = cell.sapProdID.value;

                eor.data.sold_to = cell.SAPID.value;

                eor.data.ibase_product_hier = cell.productHier.value;

                //alert('prodHier is '+eor.data.ibase_product_hier);                

                eor.data.cust_sapid = cell.SAPID.value;

                eor.data.remoteEOSL = cell.remoteEOSL.value;

                eor.data.onsiteEOSL = cell.onsiteEOSL.value;



                eor.data.enablingPartner = cell.enabling_partner.value;

                eor.data.mfgPartner = cell.mfg_partner.value;

                eor.data.distrPartner = cell.distr_partner.value;

                eor.data.resellPartner = cell.resell_partner.value;

                eor.data.directPartner = cell.direct_partner.value;

                eor.data.corporatePartner = cell.corporate_partner.value;
...
var data = [
    ...
    //console.log(aSite);
     ...
     _selectContact:function(evt,site_table){
					...
					document.querySelectorAll('#panelManageContacts2 .mysel')[0].focus();
                    var reo = new RightNow.Event.EventObject();
                    reo.data.selectedOrg = evt._currentTarget.attributes.org_id.value;

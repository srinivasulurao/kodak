...
class ibase_search extends \RightNow\Controllers\Base {
	private $_form;
	...
	*/
	...
	public function get_ibase_list() {
		...
		$list = $this->CI->model('custom/ibase_product_model')->getIbaseList("shipto", $partner_type, $sap_partner_id, $name, $city, $street, $zip, $country, $region, $material_id, $ibase_partner_id, $internal);
	...
	public function get_province_list() {
		...
		$list = array();
		$list = $this->CI->model('custom/ibase_product_model')->getStateList($country);
		$list['status'] = 1;
		print(json_encode($list));
		return;
	}
	...
	public function get_ibase() {
		...
		$ibase = $this->CI->model('custom/ibase_product_model')->getIbaseResult($partnerFunction, $partnerID, $ibaseID);
		if(count($ibase) == 0)
			$ibase['status'] = 0;
		else
			$ibase['status'] = 1;
	...
	public function sendMeterUpdate() {
		...
		$meterResult = $this->CI->model('custom/ibase_product_model')->sendMeterUpdate($counterID, $counterValue, $readDate);
		$product = array();
		if($meterResult['error'] == 1) {
			$product['status'] = 0;
			$product['error'] = $meterResult['message'];
		} else {
			$product = $this->CI->model('custom/ibase_product_model')->getProducts($productID, 'compid', $sapid, '00000002');
			$product['status'] = 1;

			$site = $this->CI->model('custom/ibase_product_model')->getSite($product[0]['SAPID']);
			$payer = $this->CI->model('custom/ibase_product_model')->getSite($product[0]['PAYERID']);
	...
	public function get_product() {
		...
		$product = $this->CI->model('custom/ibase_product_model')->getProducts($productID, $searchBy, $sapPartnerID, $partnerType);
		...
		if($product[0]['error'] && $product[0]['error']['message'] == 'MULTIPLES FOUND') {
			$product = $this->CI->model('custom/ibase_product_model')->getIbaseListByProduct($searchBy, $productID); 
		}
		if($product == null)  //when no result is found a null is returned
			...
			$product['status'] = 1;
		$site = $this->CI->model('custom/ibase_product_model')->getSite($product[0]['SAPID']);
		//CIH: 22/12/2014: Nitesh S: code changes for Payer Info to get site details from OutSide ENT Id
		$payer = $this->CI->model('custom/ibase_product_model')->getSite($product[0]['OUTSIDEENTID']);
		$payer['OUTSIDEENTID'] = $product[0]['OUTSIDEENTID'];
		//Code ends
		$payer['SAPID'] = $product[0]['PAYERID'];
		$product['Site'][0] = $site;
		$product['Payer'] = $payer;

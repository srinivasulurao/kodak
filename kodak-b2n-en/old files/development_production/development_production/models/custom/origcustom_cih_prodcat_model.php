...
class Custom_CIH_Prodcat_model extends Model
{
    ...
    */
    ...
    {
        define('queryLimit', 2);
        logmessage("call::function getHierItemsForLevel($filterType, $level)");
     ...
     */
    function hierMenuGet_old($filterType, $level, $id, $linking)
    {
        logmessage("call::function hierMenuGet($filterType, $level, $id, $linking)");
        ...
        //2012.06.22 scott harris: to enforce linked categories defined in admin interface for repair request categories
/*
...
if($prodid == 0) {
  $categoryIDs = ''; //$this->getCategoriesLinkedTo(3826);
}
else {
  $categoryIDs = sprintf("AND h.id in (%s) ", $this->getCategoriesLinkedTo($prodid));
        ...
        $sql =
            "SELECT h.id, l.label, h.seq, $childCountQuery
            FROM hier_menus h
            JOIN labels l ON l.label_id = h.id $categoryIDs AND l.tbl = $hierMenuTableID AND l.lang_id = $languageID AND l.fld = 1
            JOIN visibility v ON v.id = h.id $categoryIDs AND v.interface_id = $interfaceID AND v.tbl = $hierMenuTableID AND v.enduser = 1
            WHERE h.hm_type = $hierMenuType AND h.id=h.lvl{$level}_id $parentClause $categoryIDs
            ORDER BY h.seq, h.id, l.label";
/*
        $sql =
            "SELECT h.id, l.label, h.seq, $childCountQuery
...
*/
   ...
   /**
    ...
    {
        logmessage("call::function hierMenuGetLinkingWithLevel($id)");
        $linkMap = array(array());
        $languageID = lang_id(LANG_DIR);
   ...
   /**
    ...
    {
        logmessage("call::function hierMenuGetLinking($id)");
    ...
    {
        logmessage("call::function getEnduserVisibility($id)");
        //$interfaceID = intf_id();
        $interfaceID = 17;

        return sql_get_int(sprintf('SELECT enduser FROM visibility WHERE interface_id = %d AND tbl = %d AND id = %d', $interfaceID, TBL_HIER_MENUS, $id));
    }
    ...
    {
        logmessage("call::function getEnduserVisibleHierarchy($hierarchyArray)");
    ...
    */
    ...
    {
        logmessage("call::function getDefaultProduct()");
    ...
    */
    ...
    {
        logmessage("call::function setDefaultProduct($newDefault)");
    ...
    {
        logmessage("call::function getHierPopup($hmType, $linkingValue = null)");
   ...
   /**
    ...
    {
        logmessage("call::function convertToChain($level, $id, $filterType, $returnDataIfNotVisible = false)");
...
*********************************************************/
    ...
    {
        logmessage("call::function getCategoriesLinkedTo($id)");

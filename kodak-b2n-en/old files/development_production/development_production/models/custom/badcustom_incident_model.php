...
class custom_incident_model extends Model
{
    ...
    function createIncident($values,$thread = null)
    {
        ...
        if ($values['ek_type']=="Repair") {
            $this->CI->load->model("custom/credit_check_model");
            $this->CI->credit_check_model->repairRequestCreditCheck($c_id, $values);
        }
...
//logmessage("ek_remote_access_perm is $txt");					
        ...
        if($values['ek_ibase_updt_type'])
        {
            ...
            $incident->CustomFields->ek_ibase_updt_type->ID = $values['ek_ibase_updt_type'];
                ...
                $all_contacts = array();
                $query = sprintf("select organization from organization where A.c\$ek_customer_sapid in (%s)", rtrim($list_partners,",") == "" ? "''" : rtrim($list_partners,","));

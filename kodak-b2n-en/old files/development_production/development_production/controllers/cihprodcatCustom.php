...
class cihprodcatCustom extends ControllerBase
{
    ...
    {
        $this->load->model('custom/custom_cih_prodcat_model');
        ...
        $results = $this->custom_cih_prodcat_model->hierMenuGet($filter, $level, $id, $linking, $prodid);
    ...
    function getCatHier()
    {
        ...
        $hierData = array();
        $results = $this->custom_cih_prodcat_model->hierCatByProd($hierData[0]);
    ...
    function getIbaseProduct() 
    {
      ...
      for($i = 0; $i < count($hierItems) + 1; $i++)
      {
            ...
            if($i <= 5)
            {
                ...
                $hierData = $this->custom_cih_prodcat_model->hierMenuGet($filterName, $i+1, $arrayIndex, 0);
    ...
    {
        $this->custom_cih_prodcat_model->setDefaultProduct($prodArray);
    }
	...
	function getMap()
    {
        $this->load->model('custom/custom_cih_prodcat_model');
        $id = $this->input->request('id');
        $results = $this->custom_cih_prodcat_model->hierMenuGetLinking($id);
        sendCachedContentExpiresHeader();
        echo json_encode($results);
    }

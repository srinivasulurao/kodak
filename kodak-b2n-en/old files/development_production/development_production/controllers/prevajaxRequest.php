...
class ajaxRequest extends ControllerBase
{
    ...
    function getReportData()
    {
        ...
        $this->load->model('custom/custom_contact_model');

        $internal = $this->custom_contact_model->checkInternalContact($this->session->getProfileData('c_id'));
        ...
        if($internal == 'Y' && is_object($filters->CustomTextInput_14->filters->data) && is_object($filters->org->filters->data)) {
          ...
          if($filters->CustomTextInput_14->filters->data->val != "") {
            $requestedOrgID = $this->custom_contact_model->getOrgBySAPID($filters->CustomTextInput_14->filters->data->val);
	...
	//$this->load->model('custom/Custom_Report_model');
	$this->load->model('standard/Report_model');

        if($filters['search'] == 1)
            $this->Report_model->updateSessionforSearch();
		
        //$results = $this->Custom_Report_model->getDataHTML($reportID, $reportToken, $filters, $format);
        $results = $this->Report_model->getDataHTML($reportID, $reportToken, $filters, $format);
        /*
    ...
    */
    function getGuidedAssistanceTree()
    {
        ...
        $this->load->model('standard/Guidedassistance_model');
        sendCachedContentExpiresHeader();
        echo json_encode($this->Guidedassistance_model->getAsArray($guideID, $langID));
    }
    ...
    function getCombinedSearchResults()
    {
        ...
        if ($socialRequest)
        {
            ...
            $socialResults->searchResults = $this->Social_model->formatSearchResults(
    ...
    function getIntentGuideData()
    {
        ...
        $this->load->model('standard/Intentguide_model');
        
        echo json_encode($this->Intentguide_model->getSearchResults($keyword, array(
            'highlight' => $highlight, 
            'truncate' => $truncate, 
            'siteref' => $siteref, 
            'referrer' => $referrer, 
            ...
            'callback' => function($result, $key) use ($keyword, $truncate, $highlight, $cutoff) {
                if(($key + 1) <= $cutoff && $result->question->type === 'Answer' && !$result->answers[0]->teaser)
                {
                    ...
                    if($answer = $CI->Answer_model->get($result->answers[0]->answerID, new HTMLFormatter($keyword, true)))
                    {
    ...
    function recordIntentGuideInteraction()
    {
        ...
        $this->load->model('standard/Intentguide_model');
        $this->Intentguide_model->recordGuideInteraction($questionID, $answerID, $guideInteractionGuid, $referrer);
    }
    ...
    function getCommunityData()
    {
        ...
        $modelResults = $this->Social_model->getCachedSearchResults($keyword, $limit, null, $resourceID, null, $start, $paginateResults);
        if($paginateResults)
        {
            $modelResults->searchResults = $this->Social_model->formatSearchResults($modelResults->searchResults, $truncateSize, $highlight, $keyword, $baseUrl);
            $results['data'] = $modelResults;
        }
        else
        {
            $results['data'] = $this->Social_model->formatSearchResults($modelResults, $truncateSize, $highlight, $keyword, $baseUrl);
        }
        sendCachedContentExpiresHeader();
        echo json_encode($results);
    }
    ...
    */
    function getAnswer()
    {
        ...
        $answerID = $this->input->post('answerID');
        $this->load->model('standard/Answer_model');
        ...
        echo json_encode($this->Answer_model->get($answerID, new HTMLFormatter()));
    }
    ...
    */
    function getCommunityPost()
    {
        ...
        $this->load->model('standard/Social_model');
        sendCachedContentExpiresHeader();
        echo json_encode($this->Social_model->getCachedCommunityPost($postID));
    }
    ...
    */
    function getPostComments()
    {
        ...
        $this->load->model('standard/Social_model');
        sendCachedContentExpiresHeader();
        echo json_encode($this->Social_model->getCachedPostComments($postID));
    }
    ...
    function submitAnswerCommentAction()
    {
        ...
        $answerID = $this->input->post('answerID');
        $this->load->model('standard/Social_model');
        echo json_encode($this->Social_model->performAnswerCommentAction($answerID, $action, $data));
    }
    ...
    function submitPostCommentAction()
    {
        ...
        $this->load->model('standard/Social_model');
        echo json_encode($this->Social_model->performPostCommentAction($postID, $action, $data));
    }
    ...
    function submitCommunityPost()
    {
        ...
        $this->load->model('standard/Social_model');
        echo json_encode($this->Social_model->submitPost($postTypeID, $resourceHash, $title, $body));
    }
    ...
    function emailAnswer()
    {
        ...
        echo json_encode($this->Field_model->emailToFriend($to, $name, $from, $answerID));
    }
    ...
    function emailPassword()
    {
        ...
        $result = $this->Contact_model->emailContactPassword($login);
    ...
    function emailUsername()
    {
        ...
        $result = $this->Contact_model->emailContactLogin($email);
        echo json_encode($result);
    }
    ...
    function submitFeedback()
    {
        ...
        if($submitfeedback)
        {
            $results = $this->Incident_model->submitFeedback($answerID, $rate, $threshold, $name, $message, $givenEmail);
    ...
    function submitAnswerFeedback()
    {
        ...
        if (count($_POST))
        {
            $results = $this->Incident_model->submitFeedback($answerID, $rate, $threshold, $name, $message, $givenEmail, $optionsCount);
        }
        echo json_encode($results);
    }
    ...
    function answerNotification()
    {
        ...
        $results = $this->Contact_model->answerNotification($answerID, $status, $cid);
        echo json_encode($results);
    }
    ...
    {
        AbuseDetection::check();
        $this->load->model('standard/Notification_model');
        $values = $this->input->post('chain');
        $filter = $this->input->post('filter_type');
        //user isn't logged in--coming from email link
        $cid = $this->input->post('cid');
        $results = $this->Notification_model->add($filter, $values, $cid);
        echo json_encode($results);
    }
    ...
    function prodcatDeleteNotification()
    {
        ...
        //user isn't logged in--coming from email link
        $cid = $this->input->post('cid');
        $results = $this->Notification_model->del($hm, $cid);
        echo json_encode($results);
    }
    ...
    function prodcatRenewNotification()
    {
        ...
        $time = $this->input->post('timestamp');
        $hm = $filter . ":" . $values . ":" . $time;
        $results = $this->Notification_model->renew($hm);
        echo json_encode($results);
    }
    ...
    */
    function resetPassword()
    {
        ...
        $this->load->model('standard/Field_model');
        $results = $this->Field_model->resetPassword($data, $passwordParameter);
        echo json_encode($results);
    }
    ...
    */
    function checkForExistingContact()
    {
        ...
        $results = $this->Contact_model->contactAlreadyExists($paramType, $param, $pwReset);
        echo json_encode($results);
    }
    ...
    function doLogout()
    {
        ...
        $result = $this->Contact_model->doLogout($currentUrl, $redirectUrl);
        echo json_encode($result);
    }
    ...
    */
    ...
    {
        $this->load->model('standard/Chat_model');
        $result = $this->Chat_model->makeChatRequest();
        if($result)
            echo $result;
    }
    ...
    */
    ...
    {
        $this->load->model('standard/Clickstream_model');
        $type = $this->input->post('type');
        $widget = $this->input->post('widget');
        $column = $this->input->post('column');
        $action = (object)array('w'=>$widget.'', $column=>1);
        $this->Clickstream_model->insertWidgetStats($type, $action);
    }
    ...
    */
    function submitPoll()
    {
        $this->load->model('standard/Polling_model');
        ...
        {
            $result = $this->Polling_model->getPollResults($flowID, $questionID, $testMode);
            echo json_encode($result);
        }
        else
        {
            ...
            if (!$testMode)
                $this->Polling_model->submitPoll($flowID, $docID, $questionID, $responses, $questionType, false);
            
            if ($includeResults)
                echo json_encode($this->Polling_model->getPollResults($flowID, $questionID, $testMode));
        }
    }
    ...
    private function _requestAsyncIntentGuideData()
    {
        ...
        try {
            return $this->Intentguide_model->request('getSearchResults', $keyword, array(
                'highlight' => $highlight, 
                'truncate' => $truncate, 
                'siteref' => $siteref, 
                'referrer' => $referrer, 
    ...
    private function _requestAsyncCommunitySearch()
    {
        ...
        return $this->Social_model->request('performSearch', $keyword, $limit, null, $resourceID, null, $start, true);
    }
}

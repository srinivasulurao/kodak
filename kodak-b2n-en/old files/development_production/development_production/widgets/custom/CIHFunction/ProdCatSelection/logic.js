...
RightNow.Widget.ProdCatSelection = function(data, instanceID)
{
    this.data = data;
    ...
    RightNowEvent.subscribe('evt_resetForm', this.onResetForm, this);
    ...
    if(this.data.attrs.data_type === "products")
      RightNowEvent.subscribe("evt_setProductSelection", this._setProductSelection, this);

    if(this.data.attrs.data_type === "categories")
      RightNowEvent.subscribe("evt_setProductSelection", this._setCategory, this);
...
RightNow.Widget.ProdCatSelection.prototype = {
    /**
    ...
    */
    ...
    {
      if(this.data.attrs.data_type === "categories") {  //we want to make product read-only so, show but don't respond
    ...
    _overrideAjaxHier2Method: function () {
        RightNow.Event.unsubscribe('on_before_ajax_request', function (evt, eo) {
  //         if(eo[0].data.eventName == "evt_getHier") {
             eo[0].url = "/cc/cihprodcatCustom/getHierValues2";
  //         }
        });
  ...
  //         }
    ...
    _overrideAjaxHierMethod: function () {
        RightNow.Event.unsubscribe('on_before_ajax_request', function (evt, eo) {
  //         if(eo[0].data.eventName == "evt_getHier") {
             eo[0].url = "/cc/cihprodcatCustom/getHierValues";
  //         }
        });
  ...
  //         }
    ...
    _overrideAjaxMethod: function () {
        RightNow.Event.unsubscribe('on_before_ajax_request', function (evt, eo) {
  //         if(eo[0].data.eventName == "evt_getHier") {
             eo[0].url = "/cc/cihprodcatCustom/getIbaseProduct";
  //         }
        });
  ...
  //         }
     ...
     * @param args Object Event arguments
     * args[0].data.value contains the csv list of product hier corresponding to the ibase equipment selected
     */
    _setProductSelection: function(type, args)
    {
        this._overrideAjaxMethod();
        var postData = {"product_list": args[0].data.value };
        
        RightNow.Ajax.makeRequest("/cc/cihprodcatCustom/getIbaseProduct", postData, {
            data: { eventName: "evt_getHier" },
            successHandler: function (myresponse) {
                var resp = RightNow.JSON.parse(myresponse.responseText);
                
                this.data.js.hierData = resp;
                this._buildTree();
            },
    ...
    _setCategory: function(type, args)
    {
      ...
      if(this.data.attrs.data_type === "categories") {
      var prod_list = args[0].data.value;
      var prod = prod_list.split(",");
      var idx = prod.length - 1;

      //alert("list = " + prod_list + "prod: " + prod[idx]);
      this._selectedProduct = prod[idx];

      var postData = {"filter": "categories", "lvl": "1", "id": "0", "linking": "1", "prodid": prod[idx]};
      this._overrideAjaxHier2Method();

      RightNow.Ajax.makeRequest("/cc/cihprodcatCustom/getHierValues2", postData, {
            data: { eventName: "evt_getHier" },
            successHandler: function (myresponse) {
                var resp = RightNow.JSON.parse(myresponse.responseText);

                this.data.js.hierData = resp;
                this.data.js.hierDataNone = resp;
        ...
        _cihOnMenuFilterRequest: function (type, eventObject)
        {
                ...
                if(this.data.attrs.data_type === "categories") {
                  var postData = {"filter": eventObject.data.data_type, "lvl": (eventObject.data.level + 1), "id": eventObject.data.value, "linking": eventObject.data.linking_on, "prodid": this._selectedProduct};
                  this._overrideAjaxHierMethod();
                  RightNow.Ajax.makeRequest("/cc/cihprodcatCustom/getHierValues", postData, {"successHandler": this._cih_menuFilterGetSuccess, "scope":this, "data": eventObject, "type":"GETPOST"});
                }
        },
...
_cih_menuFilterGetSuccess: function(o)
    ...
    _getSubLevelResponse: function(type, args)
    {
        ...
        //2012.06.21 scott harris: only check orig requestor, this eliminates cross-talk :0
    ...
    */
    ...
    onResetForm: function (evt, args) {
      ...
      var eo_reset = new RightNow.Event.EventObject();
      eo_reset.w_id = this.instanceID;
      eo_reset.data = {"reset_linked_category" : true, "data_type" : "categories", "reset" : true};
      this._selected = false;
      this._nodeBeingExpanded = false;
      RightNow.Event.fire("evt_menuFilterGetResponse", eo_reset);

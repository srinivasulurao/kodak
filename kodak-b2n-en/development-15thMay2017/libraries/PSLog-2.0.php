 ...
 * Blob Hash: $Id: 0a6b46f4f326c6e8c2c49f5c56e34d51ab51d89e $
 *
...
namespace PS\Log\v2\Severity;
namespace PS\Log\v2\Type;
...
class Log extends LogBase {
  ...
  public function addConstantReference(&$obj) {
    if (\is_array(Log::constantReferences)) {
      static::$constantReferences[] = $obj;
    }
    else if (\is_object(Log::constantReferences)) {
        $arr = array (Log::constantReferences, $obj);
        static::$constantReferences = $arr;
      }
    else {
      static::$constantReferences = array ($obj);
    }
  }
  ...
  protected function _addEntry($message, $severity, $references, $note, $subtype, $type, $source) {
    ...
    $messageStr = preg_replace_callback("/[^\x0a\x0d\x20-\x7E]/", function($matches) { return "\\x".bin2hex($matches[0]); }, $messageStr);
    ...
    if ($note !== null && $note !== "" && !(is_array($note) && empty($note))) {
      $_note = preg_replace_callback("/[^\x0a\x0d\x20-\x7E]/", function($matches) { return "\\x".bin2hex($matches[0]); }, print_r($note, true));
      $entryObj->Note = $_note;
    }

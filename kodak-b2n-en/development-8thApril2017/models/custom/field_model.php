...
class Field_model extends \RightNow\Models\Base {
    ...
    */
    function resetPassword(&$data, $pwReset) {
        ...
        if($pwReset) {
            ...
            if(is_array($contactCredential)) {
                ...
                if($contactID > 0) {
                    //contact update
                    $contactsObject = $CI->model('Contact')->get(intval($contactCredential[0]));
                    $prevData = $contactsObject->toPairData(null);
                } else {
                    //contact create
                    $contactCreate = true;
                    $contactsObject = $CI->model('Contact')->getBlank();
                ...
                if(isPta() && !getConfig(PTA_IGNORE_CONTACT_PASSWORD)) {
                    ...
                    $status['contact']['create'] = $CI->model('Contact')->create($contactsObject);
                    ...
                    if(is_int($status['contact']['create']) && !is_null($contactsObject->login->value)) {
                        $profile = $CI->model('Contact')->getProfileSid($contactsObject->login->value, $contactsObject->password_new->value, $CI->session->getSessionData('sessionID'));
                        if($profile !== null && !is_string($profile))
                            $this->session->createProfileCookie($profile);
                    }
                } else {
                    $status['contact']['update'] = $CI->model('Contact')->update($contactsObject, $prevData, true);
                }
    ...
    function sendForm(&$data, &$incidentID, &$smartAsst)
    {
        ...
        if($profile !== null)
        {
            $contactsObject = $CI->model('custom/Contact_model')->get($profile->c_id->value);
            $contactPrevData = $contactsObject->toPairData(null);
        }
        else
        {
            $contactsObject = $CI->model('custom/Contact_model')->getBlank();
        }
        if($incidentID)
        {
            $incidentsObject = $CI->model('Incident')->get($incidentID);
            $incidentPrevData = $incidentsObject->toPairData();
        }
        else
        {
            $incidentsObject = $CI->model('Incident')->getBlank();
        }
        ...
        if ($contacts > 0)
        {
            if($profile === null)
            {
                if($emailFieldSeen && $contactsObject->email->value && $incidents)
                {
                    ...
                    list($existingContactID, $existingOrgId) = $CI->model('custom/Contact_model')->lookupContactAndOrgIdByEmail($contactsObject->email->value, $firstNameValue, $lastNameValue);
                    ...
                    {
                        $status['contact']['create'] = $CI->model('custom/Contact_model')->create($contactsObject);
                    }
                }
                else
                {
				die("profile empty 3");
                    $status['contact']['create'] = $CI->model('custom/Contact_model')->create($contactsObject, $passwordNewPresent);
                }
            }
            else
            {
			die("trying update");
                $status['contact']['update'] = $CI->model('custom/Contact_model')->update($contactsObject, $contactPrevData, $passwordNewPresent);
            }
        }
        ...
        if(is_int($status['contact']['create']) && !is_null($contactsObject->login->value))
        {
            $profile = $CI->model('custom/Contact_model')->getProfileSid($contactsObject->login->value, $password, $CI->session->getSessionData('sessionID'));
            if($profile !== null && !is_string($profile))
            {
                $this->session->createProfileCookie($profile);
        ...
        if($incidents > 0)
        {
            ...
            else if ($profile === null || is_string($profile))
            {
                ...
                if (is_string($disabledMessage = $CI->model('custom/Contact_model')->checkValidAccount($contactsObject)))
                {
            ...
            if($contactID)
            {
                ...
                if($incidentID)
                {
                    ...
                    $status['incident']['update'] = $CI->model('Incident')->update($incidentsObject, $incidentPrevData);
                }
                else
                {
                    ...
                    $status['incident']['create'] = $CI->model('Incident')->create($incidentsObject, $smartAsst);

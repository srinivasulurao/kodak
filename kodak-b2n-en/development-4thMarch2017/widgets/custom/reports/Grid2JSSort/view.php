...
<rn:meta controller_path="custom/reports/Grid2" js_path="custom/reports/Grid2JSSort" presentation_css="widgetCss/Grid2.css" base_css="custom/reports/Grid2" compatibility_set="November '09+"/>
...
<? 
$message=$this->data['tableData']['data'][$i][$j];
$str= explode('"',$message);
$value=trim($str[1]);
?>	
<td><? if($value):?>
       <?='<a href="'.($value ? $value.'/corp_id/'.getUrlParm('corp_id') : '&nbsp;').'">'?><?=$this->data['attrs']['url_text']?></a>
    <? endif;?>
</td>
					<? else:?>

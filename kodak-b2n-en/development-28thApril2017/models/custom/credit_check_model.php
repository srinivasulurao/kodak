...
*/ 
class credit_check_model extends \RightNow\Models\Base {
    // PSLog instance
    private $log;
	private $type;
    static $payer;
            ...
            if ($contact!==NULL) {
                ...
                if ($org!==NULL) {
                    ...
                    if ($org->CustomFields!==NULL) {
						$ci=&get_instance();
                        $payer_id = $ci->session->getSessionData("Payerinfo");
    ...
    */
	...
	{            
           ...
           $ci=&get_instance();
           $ci->session->setSessionData($arr); //Setting data into Session.

...
if($contact){
    ...
    $product=$ci->model("custom/profile_manager_model")->getProduct($prod_id);
    $category=$ci->model("custom/profile_manager_model")->getCategory($cat_id);	
}
else{
	$prod_id=(getUrlParm('p'))?getUrlParm('p'):"";
	$cat_id=(getUrlParm('c'))?getUrlParm('c'):"";
	$search_text=(getUrlParm('kw'))?getUrlParm('kw'):"";
	$lines_per_page=15; 
	$product=$ci->model("custom/profile_manager_model")->getProduct($prod_id);
    $category=$ci->model("custom/profile_manager_model")->getCategory($cat_id);
}

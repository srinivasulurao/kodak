...
class AjaxCustom extends \RightNow\Controllers\Base {
    ...
    function getProductHier() {
        ...
        $results = $this->model('custom/custom_prodcat_model')->getEnduserVisibleHierarchy($prod_arr);
    ...
    function getHierValues() {
        ...
        $results = $this->model('custom/custom_prodcat_model')->hierMenuGet($filter, $level, $id, $linking);
    ...
    function getIbaseProduct()  {
		...
		for($i = 0; $i < count($hierItems) + 1; $i++) {
			...
			if($i <= 5) {
				...
				$hierData = $this->model('custom/custom_prodcat_model')->hierMenuGet($filterName, $i+1, $arrayIndex, 0);
    ...
    function setDefaultProduct() {
        $this->model('custom/custom_prodcat_model')->setDefaultProduct($prodArray);
    }
 
	function getMap() {
        $id = $this->input->request('id');
        $results = $this->model('custom/custom_prodcat_model')->hierMenuGetLinking($id);
        sendCachedContentExpiresHeader();
        echo json_encode($results);
    }
}
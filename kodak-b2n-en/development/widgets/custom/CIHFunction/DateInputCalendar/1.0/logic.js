...
Custom.Widgets.CIHFunction.DateInputCalendar = RightNow.SearchFilter.extend({
    /**
     * Widget constructor.
     */
    overrides: {
    ...
    _setFilter: function() {
        ...
        this._eo.filters = {
            "searchName": 'custom' + this.instanceID,
            "rnSearchType": 'filter',
    ...
    _getValue: function() {
        if (this._fromDate.value === "" && this._toDate.value === "") {
            return "";
        } else if (this._fromDate.value === "" && this._toDate.value !== "") {
            var arrToDate = this._toDate.value.split("-");
            var tempDate = (parseInt(arrToDate[0]) - 100).toString() + "-" + arrToDate[1] + "-" + arrToDate[2];
            var arrFromDate = tempDate.split("-");
        } else if (this._fromDate.value !== "" && this._toDate.value === "") {
            var arrFromDate = this._fromDate.value.split("-");
            var tempDate = (parseInt(arrFromDate[0]) + 100).toString() + "-" + arrFromDate[1] + "-" + arrFromDate[2];
            var arrToDate = (tempDate.split("-"));
        } else {
            var arrFromDate = this._fromDate.value.split("-");
            var arrToDate = this._toDate.value.split("-");
        }
        //date hrs relative to 0600, ie. 6am, zero-indexed so 05=6am
        return this._stringToDate(parseInt(arrFromDate[1]), parseInt(arrFromDate[2]), arrFromDate[0], -6, 0) + '|' + this._stringToDate(parseInt(arrToDate[1]), parseInt(arrToDate[2]), arrToDate[0], 17, 59);
    },

RightNow.namespace('Custom.Widgets.login.LoginDialog2');
Custom.Widgets.login.LoginDialog2 = RightNow.Widgets.LoginDialog.extend({ 
    /**
        ...
        if(response.success == 1)
        {
            ...
            if (this._isSocialAction && this._formContainer) {
                ...
                if (RightNow.Event.fire("evt_hasSocialUserRequest", eventObject)) {
                    // fire an ajax request to determine if contact has a social user
                    RightNow.Ajax.makeRequest(this.data.attrs.has_social_user_ajax, eventObject.data, {
                        successHandler: this._onHasSocialUserResponse,
                        scope: this,
                        data: eventObject,
                        json: true
                    });
    ...
    _createSocialUserForm: function(description) {
        var templateData = {domPrefix: this.baseDomID,
                ...
                labelRequiredField: RightNow.Interface.getMessage("FIELD_REQUIRED_MARK_LBL"),
                labelRequired: RightNow.Interface.getMessage("REQUIRED_LBL")
            },

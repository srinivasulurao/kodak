...
RightNow.Widget.ProductCategorySearchFilter1 = function(data, instanceID)
{
    this.data = data;
    ...
    RightNowEvent.subscribe("evt_getFiltersRequest", this._getFiltersRequest, this);
    ...
    RightNowEvent.subscribe("evt_reportResponse", this._onReportResponse, this);
...
RightNow.Widget.ProductCategorySearchFilter1.prototype = {
    /**
    ...
    */
    _buildTree : function()
    {
        ...
        if(treeDiv && YAHOO.widget.TreeView)
        {
            ...
            if(this.data.attrs.show_confirm_button_in_dialog)
            {
                var confirmButton = 'rn_' + this.instanceID + '_' + this.data.attrs.filter_type + '_ConfirmButton',
                    cancelButton = 'rn_' + this.instanceID + '_' + this.data.attrs.filter_type + '_CancelButton',
                ...
                Event.addListener(confirmButton, 'click', function(){
                    this._tree.currentFocus.expanded = false;
    ...
    */
    _displaySelectedNodesAndClose: function(focus)
    {
        ...
        else
        {
            ...
            description = document.getElementById("rn_" + this.instanceID + "_TreeDescription");
            if(description)
    ...
    */
    _getFiltersRequest: function(type, args)
    {
        if(this._tree)
        {
            ...
            var filterType = this._eo.filters.searchName || "";
            ...
            if(this._currentIndex && this._currentIndex !== this._noValueNodeIndex)
            {
                var currentNode = this._tree.getNodeByIndex(this._currentIndex),
                      widgetId = this._eo.w_id,
                      hierValues,
                      level;
                ...
                if(this._eo.filters.data[0].length)
                {
                    if(this._getFiltersRequest.filterValues[filterType])
                    {
                        ...
                        // Only add a filter if it's new or we're updating a previous instance
                        for(var i = 0; i < filters.length; i++)
                        {
                            if(filters[i]['w_id'] !== widgetId && filters[i].toString() === eoFilters)
                            {
                                addFilter = false;
                                break;
                            }
                        ...
                        if(addFilter)
                        {
                            ...
                            this._getFiltersRequest.filterValues[filterType][this._getFiltersRequest.filterValues[filterType].length - 1]['w_id'] = widgetId;
                        }
                    }
                    else
                    {
                        this._getFiltersRequest.filterValues[filterType] = [this._eo.filters.data[0]];
                        this._getFiltersRequest.filterValues[filterType][0]['w_id'] = widgetId;
                    }
                }

                // Get the last filter added for the current filter type
                if(this._getFiltersRequest.filterValues[filterType])
            ...
            // No Value is currently set on this instance, but another instance has our chosen filter.
            else if(this._getFiltersRequest.filterValues[filterType] 
                    && this._getFiltersRequest.filterValues[filterType].length > 0
                    && this._getFiltersRequest.filterValues[filterType][this._getFiltersRequest.filterValues[filterType].length - 1].w_id !== this.instanceID)
            {
                this._eo.filters.data[0] = this._getFiltersRequest.filterValues[filterType][this._getFiltersRequest.filterValues[filterType].length - 1];
            }
            // No Value was selected on this instance.
            else
            {
                this._eo.filters.data[0] = [];
                this._eo.data.value = 0;
    ...
    _onResetRequest: function(type, args)
    {
        if(this._tree && RightNow.Event.isSameReportID(args, this.data.attrs.report_id) && (args[0].data.name === this.data.js.searchName || args[0].data.name === "all"))
        {
            ...
            else
            {
                ...
                //setting to no value
                this._eo.filters.data[0] = [];
                if(this._getFiltersRequest.filterValues && args[0].data.name)
                    this._getFiltersRequest.filterValues[args[0].data.name] = [];
                this._currentIndex = this._noValueNodeIndex;
            }
            this._displaySelectedNodesAndClose();
        }

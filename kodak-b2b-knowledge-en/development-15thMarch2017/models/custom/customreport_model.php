...
class CustomReport_model extends Model
{
const htSearchOpenOK = 1;
    ...
    * @param $reportID Int The report id that contains the column
    * @param $columnDefinition String The column definition
    *       e.g. 'answers.a_id', 'incidents.ref_no'
    * @return Mixed Array containing columnID and index members for the column or false if not found
    */
    function getIndexOfColumnDefinition($reportID, $columnDefinition)
    {
        $cacheKey = "Report-Get-$reportID";
        if(!($report = checkCache($cacheKey)))
        {
            $report = _report_get($reportID);
            setCache($cacheKey, $report);
        }
        if($report && $report['nodes'] && $report['nodes']['node_item0'] && $report['nodes']['node_item0']['cols'])
        {
            foreach($report['nodes']['node_item0']['cols'] as $column)
            {
                if($column['val'] === $columnDefinition)
                {
                    ...
                    return array('columnID' => $column['col_id'], 'index' => $column['display_order'] - 1);
                }
            }
        }
        return false;
    ...
    *   recordKeywordSearch - send 1 to signify that this search should
    *                     be recorded in the keyword_searches table
    *                     (search must also be set)
    ...
    *       data[i][a_id] - a_id of the item (if exists)
    *       data[i][i_id] - i_id of the item (if exists)
    *       data[i][link] - html link (if exists)
    *       data[i][drilldown] - drilldown link to next level (if exists)
    *   per_page - number to show
    *   page - current page
    *   total_num - total number
    *   total_pages - number of total pages
    *   row_num - row numbers should be added to data
    ...
    */
    ...
    {
        if ($this->preProcesssData($reportNumber, $reportToken, $filters, null))
        {
            return (isset($this->appliedFilters['keyword']->filters->data->val)) ? $this->appliedFilters['keyword']->filters->data->val : $this->appliedFilters['keyword']->filters->data;
        }
        return null;
    }
 ...
 *************************************************************************************************/
    ...
    /**
     * Runs the hooks with the incoming data;
    ...
    protected function preProcesssData($reportNumber, $reportToken, $filters, $format)
    {
        ...
        // only consider this an admin request if IS_ADMIN is set to true and we don't appear to be part of a unit test
        if ((!(IS_ADMIN && !beginsWith($_SERVER['REQUEST_URI'], '/ci/unitTest')) && $this->checkTokenError($reportToken)) || $this->checkInterfaceError())
        {
            return false;
        }
    ...
    protected function getData($showHiddenColumns)
    {
        ...
        $this->viewDefinition['error'] = $this->viewDefinition['error'] ? (getMessage(ERR_RPT_TABULATURE_FMT_RPT_ID_MSG) . " $this->reportID") : null;
     ...
     * @param $includeHiddenColumns bool {optional} whether to include hidden columns in the data
     */
    ...
    protected function getSimilarSearches()
    {
        ...
        if(getConfig(EU_SUGGESTED_SEARCHES_ENABLE) && $searchTerm)
        {
            ...
            // Need to trim whitespace at the end of this before sending it to sort_word_string.
            $searchTextStem = trim($searchTextStem);
    ...
    */
    protected function keywordSearchUpdate()
    {
        ...
        if ($this->appliedFilters['kfsyndication'])
            $source = SRCH_SYNDICATED_WIDGET;       
        else if ($this->reportIsTypeExternalDocument)
            $source = SRCH_EXT_DOC;
        ...
        if($stats['search_flag'] && $stats['record_flag'])
        {
            ...
            if (!$keywordsAlreadyUpdated) {
                 $this->CI->Clickstream_model->insertKeywords($stats);
        ...
        if(($stats['search_flag'] && $stats['record_flag']) || $stats['page']  > 0)
        {
            ...
            if (!$resultListAlreadyUpdated) {
                 $this->CI->load->model("standard/Clickstream_model");
                 $this->CI->Clickstream_model->insertResultList($stats);
    ...
    */
        ...
        if(icache_int_get(ICACHE_TOPIC_WORDS_EXIST))
        {
            if(strlen($searchWord) > 0)
            {
                ...
                $sql = sprintf("SELECT DISTINCT t.tw_type, t.a_id,
                    a.summary, t.title, t.text, t.url, a.type, a.url, a.access_mask
                    ...
                    t.title, t.text, t.url, a.type, a.url, a.access_mask
                    FROM topic_words_phrases ti, topic_words t
                    LEFT OUTER JOIN answers a ON a.a_id = t.a_id
            ...
            else
            {
                $sql = sprintf("SELECT t.tw_type, t.a_id, a.summary, t.title,
                    t.text, t.url, a.type, a.url, a.access_mask
                    FROM topic_words t
                    LEFT OUTER JOIN answers a ON a.a_id = t.a_id
                    WHERE interface_id = %d
                    AND (a.status_type = %d OR a.status_type IS NULL)
            ...
            $contactAccess = 0;
            foreach(explode(',', contact_answer_access()) as $id) {
            ...
            $topicWordData = array();
            while(list($topicType, $answerID, $summary, $title, $text, $topicUrl, $answerType, $answerUrl, $answerAccessMask) =  sql_fetch($si))
            {
                // Need to make sure that the external URLs don't get the answer access level checked, because they have none.
                if($topicType === 1 && !(intval($answerAccessMask) & $contactAccess))
                    continue;
    ...
    protected function didYouMean()
    {
        ...
        $data = array();
        if(getConfig(EU_SEARCH_TERM_FEEDBACK_ENABLE) && $searchTerm)
        {
            ...
            if (null === ($queryResults = checkCache($cacheKey)))
            {
                if ($isComplex)
                {
                ...
                setCache($cacheKey, $queryResults);
            }
 ...
 *****************************************************************************************/
    ...
    protected function getExternalSearchData()
    {
        ...
        $preHookData = array('data' => array('queryArguments' => $htSearchQueryArguments, 'reportType' => 'external'));
        RightNowHooks::callHook('pre_report_get_data', $preHookData);
        ...
        $postHookData = array('data' => array('returnData' => $this->returnData, 'reportType' => 'external'));
        RightNowHooks::callHook('post_report_get_data', $postHookData);

        setCache($cacheKey, $postHookData['data']['returnData']);
    }
 ...
 *****************************************************************************************/
    ...
    protected function convertCPFiltersToQueryArguments()
    {
        ...
        // only consider this an admin request if IS_ADMIN is set to true and we don't appear to be part of a unit test
        return array(
            'param_args' => (IS_ADMIN && !beginsWith($_SERVER['REQUEST_URI'], '/ci/unitTest')) ? array() : $this->filtersToOutputVariables(),
    ...
    protected function getReportData($showHiddenColumns)
    {
        ...
        $preHookData = array('data' => array('queryArguments' => $queryArguments, 'reportType' => 'internal'));
        RightNowHooks::callHook('pre_report_get_data', $preHookData);
        ...
        $postHookData = array('data' => array('returnData' => $this->returnData, 'reportType' => 'internal'));
        RightNowHooks::callHook('post_report_get_data', $postHookData);

        setCache($cacheKey, array($postHookData['data']['returnData'], $this->viewDataColumnDefinition));
        ...
        foreach($reportQueryData as $filter){
            if($filter['name'] === $productSearchFilterID || $filter['name'] === $categorySearchFilterID){
                ...
                foreach($itemChain as $idAndLevelCombination){
                    ...
                    $searchPerformed = true;
                    $hierMenuID = end(explode('.', $idAndLevelCombination));
    ...
    */
    ...
    protected function setMaxResultsBasedOnSearchLimiting($reportPerPageSetting)
    {
        ...
        if (!$this->appliedFilters['no_truncate'] && !($this->appliedFilters['page'] > 1) && $searchResultLimitingStyle &&
            $this->appliedFilters['keyword']->filters->data && $this->CI->session && !$this->CI->session->getProfileData('lines_per_page'))
        {
     ...
     * @param $hiddenColumnsIncluded bool Whether or not to include hidden columns as part of the return data
     */
                ...
                if ( (isset($this->viewDataColumnDefinition["col_item{$j}"]['hidden']) &&  ($this->viewDataColumnDefinition["col_item{$j}"]['hidden'] === 0)) || $hiddenColumnsIncluded)
                {
                    ...
                    else if ($bindType == BIND_DATE || $bindType == BIND_DTTM)
                    {
                        if (!is_null($row[$j]))
                        {
                            ...
                            $temp[$count] = ($formatDefine !== false) ? date_str($formatDefine, $row[$j]) : $row[$j];
                        }
                        else
                        {
    ...
    protected function filtersToOutputVariables() {
        ...
        $index = 0;
        if ($contactSessionAlias = view_tbl2alias($this->reportID, TBL_CONTACT_SESSIONS)) {
            foreach ($this->viewDefinition['all_cols'] as $column) {
                if (stringContains($column['col_definition'], '$sessionid')) {
     ...
     * @param $contactDataSet Boolean Whether to add the contact info for incidents (an org-filtered search isn't being done)
     * @param $searchArgs Array Contains all query arguments for the search
     * @param $index Int (optional) The index to add the contact info into the array
     * @return array the searchArgs modified array
     */
    protected function addContactInformation($contactDataSet, $searchArgs, $index = 0)
    {
        $contactID = ($this->CI->session && $this->CI->session->getProfileData('c_id')) ? $this->CI->session->getProfileData('c_id') : 0;
        if ($this->incidentTableAlias && !$contactDataSet)
        {
            // set contact
            $searchArgs['search_field' . $index++] = $this->toFilterArray("{$this->incidentTableAlias}.c_id", OPER_EQ, '' . $contactID);
        }

        $answerNotificationTableAlias = view_tbl2alias($this->reportID, TBL_ANS_NOTIF);
        if ($answerNotificationTableAlias)
        {
            $searchArgs['search_field' . $index++] = $this->toFilterArray('ans_notif.interface_id', OPER_EQ, intf_id() . '');
            $searchArgs['search_field' . $index++] = $this->toFilterArray("$answerNotificationTableAlias.c_id", OPER_EQ, '' . $contactID);
        }
        ...
        {
            // set contact
            if (($orgID = $this->CI->session->getProfileData('org_id')) && $orgID !== INT_NULL)
            {
                $searchArgs['search_field' . $index++] = $this->toFilterArray("$slaTableAlias.owner_id", OPER_EQ, '' . $orgID);
                $searchArgs['search_field' . $index++] = $this->toFilterArray("$slaTableAlias.owner_tbl", OPER_EQ, '' . TBL_ORGS);
            }
            else
            {
                $searchArgs['search_field' . $index++] = $this->toFilterArray("$slaTableAlias.owner_id", OPER_EQ, '' . $contactID);
                $searchArgs['search_field' . $index++] = $this->toFilterArray("$slaTableAlias.owner_tbl", OPER_EQ, '' . TBL_CONTACTS);
            }
        }

        $contactSessionAlias = view_tbl2alias($this->reportID, TBL_CONTACT_SESSIONS);
